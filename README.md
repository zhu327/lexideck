...
| 功能 | 说明 |
|------|------|
| **Review** | 翻卡 → 评分，支持 TTS 发音、快捷键（Space 翻卡，1-4 评分）、间隔反馈动画 |
| **Quiz** | 随机测验模式，浏览已学卡片 |
| **Known (已掌握)** | 标记已认识的单词，自动跳过复习队列 |
| **Search** | 全文搜索笔记，支持分页加载、编辑与删除 |
| **Stats** | 今日复习数、连续天数、留存率、总卡片数、各状态分布 |
| **Add** | 手动添加新笔记，支持选择 Deck / Model / Tags |
| **离线支持** | Service Worker 缓存 + 离线复习队列，恢复网络后自动同步 |
| **Badge 通知** | 通过 Web Badge API 在 PWA 图标上显示待复习数量 |
| **Enrichment 面板** | 复习时一键调用 LLM 生成词汇增强内容，结果自动缓存 |
